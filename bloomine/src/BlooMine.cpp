...
int main( int argc, char** argv )
{
  ...
  /*
  Declaires an instance of FastQ to handle fastq files and partitioning.
  Way easier than faffing with it in funcs here.
  */
  FastQ FQ( vm["fastq"].as<std::string>() );
  FQ.partition( vm["threads"].as<int>() );
  ...
  // read hit outfile, should probably be a user defined name tbh
  std::ofstream fp_outfile;
  fp_outfile.open( vm["prefix"].as<std::string>() + "_BMfiltered.fq", std::ios::out );

  // big gross vec of future vecs containing read hits
  std::vector<std::future<std::vector<std::string>>> hits_collection;

  vprint( "FP-SCREEN", "Starting BlooMine screening pipeline...", "g" );
  /* Spawn threads using the Thread struct
  */
  for ( int tnum=0; tnum < vm["threads"].as<int>(); tnum++ ) {
    cout << "\t\tThread created : " << tnum << "\tfqpart : " << FQ._parts[tnum] << "\n";
    std::future<std::vector<std::string>> fp = std::async( runBM, vm, FQ._parts[tnum], BF, target_kset, fp_threshold, spMST, hit, gap_open, neg );
    hits_collection.push_back(std::move(fp));
  }

  cout << "\n";

  // collect all the hits vectors from threads
                                ...
                                const double neg )
{
  ...
  catch ( std::string fq_part ) {
    vprint( "FQ-ERROR", "There was a problem opening " + fq_part, "r" );
    exit (2);
  }
...
po::variables_map parse_args( int argc, char** argv )
{
  ...
  float s = 0.0;
  float e = 0.0;
  check_argc( argc, argv );     // check the number of args provided
  ...
  try {
    ...
    // argscheck chunk
    if ( threads != 1 ) { check_threadcount( threads ); }  // just returns if running in single core mode
    check_floats( f, s );  // check the args of the false positive and FP-sim floats
    check_ksize( k );     // check the given kmer size
 ...
 ****************************************************************************************************/
void check_threadcount( int threads )
{
 ...
 ****************************************************************************************************/
void check_floats( float f, float s )
{
  /* Checks that the false postitive rate and FP-sim scores are within acceptable range
  */
  try {
 ...
 *  vm <variables_map> : a boost program options map containing argument variables
 ****************************************************************************************************/
void print_params( po::variables_map vm )
{
